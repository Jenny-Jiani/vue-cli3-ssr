export class Util {
   ...
   * @memberof Util
   */
  static distinctArray(array, compare) {
   ...
   * @memberof Util
   */
  static compareArray(array1, array2, compare) {
    if (!array1 || !array2) {
      return false
    }
   ...
   * @memberof Util
   */
  static swapArrayElement(array, first, second) {
   ...
   * @memberof Util
   */
  static contains(array, element, comparer) {
    for (let i in array) {
   ...
   * @memberof Util
   */
  static findIndex(array, element, comparer) {
    for (let i in array) {
      if (comparer && comparer(array[i], element)) {
        return i
      } else {
   ...
   * @memberof Util
   */
  static ArrayInclude(array, element, compare) {
    for (let i = 0; i < array.length; i++) {
      if (compare) {
   ...
   * @memberof Util
   */
  static removeElement(array, element, comparer) {
  ...
  static printLog(msg) {
    ...
    if (blowersVersion == -1 || blowersVersion == 'edge' || blowersVersion > 9) {
      console.log(msg)
    }
  }
  ...
  static formatDateTime(time) {
    ...
    const minuter = Dates.getMinutes() < 10 ? '0' + Dates.getMinutes() : Dates.getMinutes()
  ...
  static compareDate(beginDate, endDate) {
  ...
  // 判断信用卡属于哪个公司
  static DetectCardType(_cardNumber) {
  ...
  // 信用卡卡号校验
  static ValidateCardNumber(_cardNumber) {
    _cardNumber = _cardNumber.replace(/[\s]/g, '')
    _cardNumber = _cardNumber.replace(/[-]/g, '')
    if (_cardNumber == '') {
      ...
      if (iCardType == 0) {
        ...
        if (
          this.CheckLength(_cardNumber, iCardType) == true &&
          this.CheckDigit(_cardNumber) == true
        ) {
          return null
        } else {
  ...
  static CheckLength(strCardNum, iCardType) {
    var isValid = false
    switch (iCardType) {
      case 5: // 15
